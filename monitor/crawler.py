...
def _fetch_url(url: str, timeout: int = 15) -> requests.Response | None:
    ...
    Designed to be called via run_in_executor — never call directly
    in an async context.
    """
    try:
...
async def get_pages_to_monitor(site: dict) -> list[str]:
    """
    Returns list of URLs to check for a site.
    single_page → just the main URL.
    whole_site  → crawl/sitemap discovery.
    """
...
async def get_urls_from_sitemap(base_url: str, config: dict) -> list[str]:
    """Fetch and parse sitemap.xml, return list of filtered URLs."""
    parsed = urlparse(base_url)
    sitemap_url = f"{parsed.scheme}://{parsed.netloc}/sitemap.xml"

    loop = asyncio.get_running_loop()
    resp = await loop.run_in_executor(None, lambda: _fetch_url(sitemap_url))
...
async def crawl_site(site: dict) -> list[str]:
    """
    Crawl a site to discover pages to monitor.
    Tries sitemap first, falls back to link-following.
    ...
    while to_visit and len(found) < max_pages:
        ...
        resp = await loop.run_in_executor(None, lambda u=url: _fetch_url(u))
        if resp is None:
            continue
        ...
        # Polite crawl delay — run in executor so it doesn't block event loop
...
def filter_urls(urls: list[str], site: dict) -> list[str]:
    """Filter a URL list by the site's exclusion patterns."""
